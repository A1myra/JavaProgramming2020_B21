...
public class WrapperClassPractice {
    ...
    public static void main(String[] args) {
        String str = "a1b2c3";  // in ascii table ==> 1: 49, 2: 50, 3: 51
        int sum = 0;

        for (int i = 0; i <= str.length() - 1; i++) {
            if (Character.isDigit(str.charAt(i))) { // to verify if each char in string is a digit
                sum += Integer.parseInt("" + str.charAt(i));//each digits need to be converted to integer
            }
        }
        ...
        for (char each : password.toCharArray()) {
            if (Character.isUpperCase(each)) { // verify if the character is upper case
                ...
                lowerLetters += each;
            } else if (Character.isDigit(each)) {//verify if the character is number
                digits += each;
